...
####toggle diverters if needed
        ...
        for cord0 in self.diverter_locations:
            loc2 = copy(cord0)
            try:
                if self.carrier_type_map[cord0[1]][cord0[0]] == self.init_queues[self.diverter_locations.index(cord0)][0]: #and len(self.init_queues[self.diverter_locations.index(loc2)]) >= self.len_longest_sublist(self.get_candidate_lists(self.init_queues, self.carrier_type_map[loc2[1]][loc2[0]])): 
                    self.D_states[self.diverter_locations.index(loc2)+1] = True
                    print("set diverter state for diverter {} to TRUE".format(self.diverter_locations.index(loc2)+1))
                    self.remove_from_queue(self.diverter_locations.index(loc2)+1)
                    print("request removed from queue")

                else:
                    self.D_states[self.diverter_locations.index(loc2)+1] = False
...
####Do step for all items on conv
        for item in self.items_on_conv:
            ...
            for cord1 in self.diverter_locations:
                loc1 = copy(cord1)
                if item[0] == cord1 and self.D_states[self.diverter_locations.index(loc1)+1] == True:
                    item[0][1] +=1
