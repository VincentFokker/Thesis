...
if __name__ == "__main__":
    ...
    parser.add_argument('-r', '--retrain', action='store_true', help='If the model should be retrained after pretraining.')
    ...
    #load the data for pre-train
    # Using only one expert trajectory
    # you can specify `traj_limitation=-1` for using the whole dataset
    try:
        dataset = ExpertDataset(expert_path=join(specified_path, 'heuristic_expert.npz'),
